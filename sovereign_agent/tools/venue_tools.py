...
def generate_event_flyer(venue_name: str, guest_count: int, event_theme: str) -> str:
    ...
    prompt = (
    f"A sleek, modern event flyer for '{event_theme}'. "
    f"Venue: {venue_name}, Edinburgh, Scotland. "
    f"Capacity: {guest_count} guests. "
    f"Visual style: dark navy and gold color palette, atmospheric Scottish pub interior, "
    f"warm amber lighting, exposed stone walls, polished wood. "
    f"Typography: bold sans-serif headline 'AI MEETUP EDINBURGH' centered at top, "
    f"clean white subtext with venue name and guest count below. "
    f"Mood: professional, innovative, intimate. "
    f"No people, no faces. Photorealistic. 16:9 landscape format."
)
